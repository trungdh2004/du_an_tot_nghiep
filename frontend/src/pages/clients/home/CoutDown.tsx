...
const Countdown = ({date}:Props) => {
  ...
  const TimeUnit = ({ value, label }:{ value:string, label:string }) => (
    <div className="flex flex-col items-center">
      <span className="flex items-center justify-center text-2xl font-medium text-gray-800 bg-white rounded-full size-12 md:size-20">{value}</span>
  ...
  return (
    <div className="flex flex-col gap-2">
      <div className="flex items-center">
        ...
        <TimeSeparator />
        <TimeUnit value={String(timeLeft.minutes).padStart(2, '0')} label="Phút" />
        <TimeSeparator />
        <TimeUnit value={String(timeLeft.seconds).padStart(2, '0')} label="Giây" />
      </div>
    </div>
  );
};
