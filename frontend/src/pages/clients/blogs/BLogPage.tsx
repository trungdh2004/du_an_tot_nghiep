        ...
        <>
            ...
            <div className="padding pt-12 pb-10 text-[#1A1E26]">
                ...
                <ScrollArea className=" max-w-[100%] whitespace-nowrap">
                    <div className='space-x-3 pb-4' >
                        {tags && tags.map((tag: any, index: number) => {
                            // console.log(tag)
                            return (
                                <Button onClick={() => handleParams(tag.slug)} variant="outline" className={cn('rounded-lg border border-slate-300 pr-3', newTags === tag.slug ? 'border-blue-500 text-blue-500' : 'border-slate-300')}>{tag.name}</Button>
                            )
                        })}
                    </div>
                ...
                <div className="flex justify-center mt-5">
                    <Paginations forcePage={searchObject.pageIndex - 1} pageCount={blogs?.totalPage} handlePageClick={handleChangePag} />
                </div>
            </div >
            {/* tags-;íst */}
