  ...
  return (
    <>
      ...
      <div className="px-4 md:px-10" >
        <div className="text-base md:text-lg text-[#333333] font-medium">Các giao dịch online của bạn</div>
        <div className="grid grid-cols-12 gap-6 mt-5 ">
          {data?.content?.map((item: IPayment, index: number) => {
            return (
              <div key={index} className="border border-blue-200 bg-white rounded-[8px] px-4 py-2 col-span-12  min-[900px]:col-span-6 ">
                <p className="text-sm font-semibold pb-2">Mã đơn hàng: <span className="">{item?.codeOrder}</span></p>
                <p className="">Có giao dịch thanh toán online với số tiền
